...
| **Secrets** | `.env` file (gitignored) | Azure Key Vault with RBAC and Key Vault references by default; policy-constrained POCs can use role-specific encrypted Container Apps secrets while retaining Key Vault copies |
| **Bronze storage** | `./data/bronze` | Azure Storage account + Azure Files share mounted into the collector job; policy-constrained POCs can disable the mount and use ephemeral `/tmp/bronze` with no replay durability |
...
Bicep template at [`infra/bicep/main.bicep`](./infra/bicep/main.bicep) is a single-region Azure P1 foundation deployed with the Azure Developer CLI (`azd up`) via the [`infra/main.bicep`](./infra/main.bicep) subscription-scoped wrapper and `azure.yaml`. It has explicit image, secret, sizing, storage, migration, schedule, auth, and alerting parameters, and accepts separate DB URL secrets. Key Vault references remain the default; `useKeyVaultReferences=false` is a policy-constrained POC fallback that injects the same role-specific values as encrypted Container Apps secrets while retaining ARM-managed Key Vault copies. An `azd` postprovision hook ([`infra/hooks/postprovision.sh`](./infra/hooks/postprovision.sh)) creates a temporary current-client-IP PostgreSQL firewall rule with guaranteed cleanup, runs [`infra/postgres/bootstrap-roles.sql`](./infra/postgres/bootstrap-roles.sql) to create the least-privilege `web_readonly`, `collector_writer`, and `migration_admin` roles, runs the DB migration runner plus the production-safe seed as `migration_admin`, and verifies grants. Microsoft Entra built-in auth (`enableEntraAuth`) and Azure Monitor alerts plus an `/api/health` availability test (`enableAlerts` / `enableAvailabilityTest`) are provisioned behind toggle params. This is not a complete enterprise landing zone (public ingress, no VNet/private endpoints); a release MUST NOT claim production self-host readiness until the complete P1 checklist in spec 08 passes.
...
| **Secrets management** | P1 Azure target uses Key Vault references. Policy-constrained POCs may use encrypted Container Apps secrets with role-specific values and ARM-managed Key Vault copies; production deployments should use private networking rather than this fallback. Plain `.env` is only for local dev. |
| **Policy-constrained storage** | `enableBronzeFileShareMount=false` switches the collector to ephemeral `/tmp/bronze` when Storage data-plane access is disabled. This preserves Postgres facts but forfeits bronze replay/export durability and is not a production topology. |
